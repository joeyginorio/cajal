...
class ModelD(nn.Module):
    def __init__(self):
        ...
        self.ceq = cj.TmIf(cj.TmVar('x1'),
        (cj.TmIf(cj.TmVar('x2'), cj.TmTrue(), cj.TmFalse())), 
        (cj.TmIf(cj.TmVar('x2'), cj.TmFalse(), cj.TmTrue())))
        check(self.ceq, {'x1': cj.TyBool(), 'x2': cj.TyBool()})

    def forward(self, x1, x2):
        env = {'x1' : TypedTensor(self.netx1(x1), cj.TyBool()), 
               'x2' : TypedTensor(self.netx2(x2), cj.TyBool())}
        ceq_compiled = vmap(compile(self.ceq), 
...
with open("experiments/eq/data/direct_eq_loss_train.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("experiments/eq/data/direct_eq_loss_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "seed", "batch size", "lr", "loss"])
    for (step, seed, batch_size, lr), loss in loss_test.items():
        writer.writerow([step, seed, batch_size, lr, loss])
with open("experiments/eq/data/direct_eq_acc_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
