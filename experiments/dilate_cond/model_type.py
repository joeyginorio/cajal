...
class ModelD(nn.Module):
    def __init__(self):
        ...
        self.random_cond_iter = nn.Sequential(
            nn.Linear(7840*2,784)
        )
    ...
    def forward(self, x):
        ...
        even_val = self.is_even(x)
        ys = torch.vmap(torch.kron)(base_val, num_val)
        ys = torch.vmap(torch.kron)(ys, even_val)
        return self.random_cond_iter(ys).view(-1,28,28)
...
with open("experiments/dilate_cond/data/type_loss_train.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("experiments/dilate_cond/data/type_loss_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "seed", "batch size", "lr", "loss"])
    for (step, seed, batch_size, lr), loss in loss_test.items():
        writer.writerow([step, seed, batch_size, lr, loss])
with open("experiments/dilate_cond/data/type_output_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("experiments/dilate_cond/data/type_psnr_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
