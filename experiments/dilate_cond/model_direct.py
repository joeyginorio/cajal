...
# ---------- Data --------------------
train_xs = torch.load("experiments/dilate_cond/data/train_xs.pt")
train_ys = torch.load("experiments/dilate_cond/data/train_ys.pt")
...
class ModelD(nn.Module):
    def __init__(self):
        ...
        self.cond = cj.TmIf(cj.TmVar('is_even'),
                            cj.TmVar('og_img'),
                            self.p)

        self.cond_iter = torch.vmap(compile(self.cond),
                                    in_dims=({'base': (TypedTensor(0, None)),
                                              'is_even': (TypedTensor(0, None)),
                                              'og_img': (TypedTensor(0, None)),
                                              'iterator': None,
    ...
    def forward(self, x):
        ...
        env = {'base' : TypedTensor(base_val, cj.TyBool()),
               'is_even': TypedTensor(is_even, cj.TyBool()),
               'og_img': TypedTensor(og_img, cj.TyBool()),
               'iterator' : iterator,
               'num' : TypedTensor(num_val, cj.TyNat())}
        return self.cond_iter(env).data.view(-1,28,28)
...
# cnn = CNN().to(device)
# cnn_path = "experiments/dilate_cond/data/mnist_cnn.pt"
...
with open("experiments/dilate_cond/data/direct_loss_train.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("experiments/dilate_cond/data/direct_loss_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
    writer.writerow(["step", "seed", "batch size", "lr", "loss"])
    for (step, seed, batch_size, lr), loss in loss_test.items():
        writer.writerow([step, seed, batch_size, lr, loss])
with open("experiments/dilate_cond/data/direct_output_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
...
with open("experiments/dilate_cond/data/direct_psnr_test.csv", "w", newline="") as f:
    writer = csv.writer(f)
